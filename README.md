...
* either build from source (a build script which you can use for insipration is in this repo at `.circleci/config.yml`) or use the prebuilt binary at `prebuilt/keywriter`
* copy the binary onto your tablet `scp keywriter 10.11.99.1:/home/root/keywriter` (use the IP address and destination of your own set up). If you use the [draft](https://github.com/dixonary/draft-reMarkable) custom launcher, you'll find example config and an icon in `draft_files`.
...
* `ssh` to your remarkable and create the directory notes will be stored in
  * `mkdir -p /home/root/keywriter`
  * Add some content to the default scratch note:
  * `echo scratch > /home/root/keywriter/scratch.md`
